...
class Quiz extends MY_Controller {
	...
	public function index(){
		...
		if(count($this->data['quiz_attempts']) > 0){
			...
			if(!$this->data['next']){
				...
				$this->data['c_c_attempt'] = $this->Course_Chapter_Attempts_Model->get_all('', array(
					'c_c_attempt_caid' => @$this->data['quiz_attempts'][0]->quiz_attempt_caid,
		...
		$this->data['page_content'] = 'student/quiz/index';
	...
	public function attempt(){
		...
		$this->data['page_title'] = $this->data['page_title'] . ' | '.$this->data['course']->course_title;
		$this->data['page_content'] = 'student/quiz/attempt';
		$this->load->view('student/template', $this->data);
	}
	...
	public function process_answer(){
		...
		if($q_current >= (count($quiz['questions']) - 1)){
			...
			redirect(base_url('student/quiz/?qaid='.$quiz_attempt_id.'&course='.$course_id.'&chapter='.$chapter_id.'&tool='.$tool_id.'&quiz='.$quiz_id));
		}else{
			$this->session->set_flashdata('q_counter', $q_current+1);
		}
		
		redirect(base_url('student/quiz/attempt/?qaid='.$quiz_attempt_id.'&course='.$course_id.'&chapter='.$chapter_id.'&tool='.$tool_id.'&quiz='.$quiz_id));
