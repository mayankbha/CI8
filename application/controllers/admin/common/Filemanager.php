...
class Filemanager extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->library('model_tool_image');
	}
	public function index() {
		...
		if (substr(str_replace('\\', '/', realpath($directory . '/')), 0, strlen(DIR_IMAGE . 'catalog')) == DIR_IMAGE . 'catalog') {
			...
			// Get files
			$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF,pdf,doc,docx,odt,PDF,DOC,DOCX}', GLOB_BRACE);
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => base_url('admin/common/filemanager').'?directory=' .substr($image, strlen(DIR_IMAGE . 'catalog/')) . $url,
				);
				...
				if($ext == 'pdf'){
					$raw_image['thumb'] = $server . 'assets/images/pdf-file-icon.png';
				}
				if($ext == 'doc' || $ext == 'docx'){
					$raw_image['thumb'] = $server . 'assets/images/doc-file-icon.png';
				}
				if($ext == 'odt'){
					$raw_image['thumb'] = $server . 'assets/images/odt-file-icon.png';
				}

				$data['images'][] = $raw_image;
			}
		}
		...
		$data['parent'] = base_url('admin/common/filemanager').'?token=token'. $url;
		...
		$data['refresh'] = base_url('admin/common/filemanager').'?token=token'.$url;
